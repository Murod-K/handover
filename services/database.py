...
# Single shared connection, created once at startup
...
async def init_db():
    ...
    cur = await db.execute("SELECT COUNT(*) FROM structure_types")
...
async def _seed_defaults(db: aiosqlite.Connection):
    ...
    await db.executemany("INSERT OR IGNORE INTO projects (name) VALUES (?)", [
        ("Объект 1",), ("Объект 2",)
    ])

    cur = await db.execute("SELECT id FROM projects LIMIT 1")
    row = await cur.fetchone()
    if row:
        await db.execute("INSERT OR IGNORE INTO subprojects (project_id, name) VALUES (?,?)", (row["id"], "Блок А"))
        await db.execute("INSERT OR IGNORE INTO subprojects (project_id, name) VALUES (?,?)", (row["id"], "Блок Б"))
